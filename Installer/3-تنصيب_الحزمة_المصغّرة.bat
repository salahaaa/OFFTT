...
if not exist "%SRC%MfgSystem.exe" (
    echo        [خطأ] لم يُعثر على MfgSystem.exe — انسخ المجلد كاملاً.
    pause & exit /b 1
)
...
for %%P in ("%ProgramFiles%\%APPNAME%" "%ProgramFiles(x86)%\%APPNAME%" "%LocalAppData%\Programs\%APPNAME%") do (
    if exist "%%~P\MfgSystem.exe" echo        [تنبيه] تنصيب سابق في: %%~P
)
echo.
...
if errorlevel 1 ( echo        [تنبيه] أنشئ اختصاراً يدوياً إلى: %DEST%\MfgSystem.exe ) else (
