...
set "SRC=%~dp0"
set "FILES=MfgSystem.exe MfgSystem.dll MfgSystem.pdb MfgSystem.deps.json DatesErp.Core.dll DatesErp.Core.pdb DatesErp.Application.dll DatesErp.Application.pdb DatesErp.Infrastructure.dll DatesErp.Infrastructure.pdb"
...
if not exist "%DEST%\MfgSystem.exe" (
    echo [خطأ] لا يوجد MfgSystem.exe في:  %DEST%
...
powershell -NoProfile -Command "Get-FileHash -Algorithm SHA256 '%DEST%\MfgSystem.dll','%DEST%\MfgSystem.exe','%DEST%\DatesErp.Application.dll','%DEST%\DatesErp.Infrastructure.dll','%DEST%\DatesErp.Core.dll' | ForEach-Object { '{0}  {1}' -f $_.Hash, (Split-Path $_.Path -Leaf) }"
echo.
echo ══════════════════════════════════════════════════════════
