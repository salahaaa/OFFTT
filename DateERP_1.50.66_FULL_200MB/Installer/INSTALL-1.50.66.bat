@echo off
REM =====================================================================
REM  DateERP 1.50.66 - ONE CLICK INSTALLER
...
set "LOG=%INSTALLER_ROOT%\install_1.50.66.log"
...
if /I "%INSTALL_ROOT%"=="D:\DateERP_Publish" set "INSTALL_ROOT=D:\DateERP_1.50.66"
if /I "%INSTALL_ROOT%"=="C:\DateERP_Publish" set "INSTALL_ROOT=C:\DateERP_1.50.66"
if /I "%INSTALL_ROOT%"=="C:\DateERP" set "INSTALL_ROOT=C:\DateERP_1.50.66"
if /I "%INSTALL_ROOT%"=="D:\DateERP" set "INSTALL_ROOT=D:\DateERP_1.50.66"
...
if %errorlevel% NEQ 0 (
  echo [FAILED] Step 6: copy failed. See install_1.50.66.log
...
set "CONFIG_DEST=%INSTALL_ROOT%\Config\appsettings.1.50.66.json"
...
REM Ensure active config points to isolated DB DateERP_1_50_66 and isolated paths
REM If appsettings.json already has correct Version, keep it, else create new one
powershell -NoProfile -Command ^
  "$jsonPath='%CONFIG_ACTIVE%'; $isolatedDb='DateERP_1_50_66'; $ver='1.50.66'; $installRoot='%INSTALL_ROOT%'; " ^
...
if %errorlevel% NEQ 0 (
  ...
  (
    ...
    echo     "DefaultConnection": "Server=.\SQLEXPRESS;Database=DateERP_1_50_66;Trusted_Connection=True;MultipleActiveResultSets=true;TrustServerCertificate=True"
    echo   },
    echo   "AppSettings": {
    echo     "Version": "1.50.66",
...
powershell -NoProfile -Command "try { $j=Get-Content '%INSTALL_ROOT%\publish\appsettings.json' -Raw | ConvertFrom-Json; if ($j.AppSettings.Version -eq '1.50.66') { exit 0 } else { exit 1 } } catch { exit 1 }" >nul 2>&1
if %errorlevel%==0 set "VER_OK=1"
...
powershell -NoProfile -Command ^
  "$WshShell=New-Object -ComObject WScript.Shell; $lnk=$WshShell.CreateShortcut('%DESKTOP%\DateERP 1.50.66.lnk'); $lnk.TargetPath='%INSTALL_ROOT%\publish\MfgSystem.exe'; $lnk.WorkingDirectory='%INSTALL_ROOT%\publish'; $lnk.Description='DateERP %VERSION% - Isolated Install'; $lnk.Save()" >> "%LOG%" 2>&1
if %errorlevel%==0 (
...
powershell -NoProfile -Command ^
  "$WshShell=New-Object -ComObject WScript.Shell; $lnk=$WshShell.CreateShortcut('%STARTMENU%\DateERP 1.50.66.lnk'); $lnk.TargetPath='%INSTALL_ROOT%\publish\MfgSystem.exe'; $lnk.WorkingDirectory='%INSTALL_ROOT%\publish'; $lnk.Description='DateERP %VERSION%'; $lnk.Save()" >> "%LOG%" 2>&1
...
powershell -NoProfile -Command ^
  "$WshShell=New-Object -ComObject WScript.Shell; $lnk=$WshShell.CreateShortcut('%STARTMENU_USER%\DateERP 1.50.66.lnk'); $lnk.TargetPath='%INSTALL_ROOT%\publish\MfgSystem.exe'; $lnk.WorkingDirectory='%INSTALL_ROOT%\publish'; $lnk.Description='DateERP %VERSION%'; $lnk.Save()" >> "%LOG%" 2>&1
...
(
  ...
  echo echo Config: %INSTALL_ROOT%\Config\appsettings.1.50.66.json
  ...
  echo   echo Please re-run INSTALL-1.50.66.bat as administrator.
  echo   pause
  echo   exit /b 1
  echo ^)
...
powershell -NoProfile -Command "$sh=New-Object -ComObject WScript.Shell; $lnk=$sh.CreateShortcut('%DESKTOP%\DateERP 1.50.66.lnk'); if ($lnk.TargetPath -like '*DateERP_Publish*' -or $lnk.TargetPath -like '*C:\DateERP\*' ) { exit 1 } else { exit 0 }" >nul 2>&1
...
dotnet publish "%SLN_DIR%src\DatesErp.Desktop\DatesErp.Desktop.csproj" -c Release -o "%PUBLISH_SRC%" -v m >> "%LOG%" 2>&1
if %errorlevel% NEQ 0 (
