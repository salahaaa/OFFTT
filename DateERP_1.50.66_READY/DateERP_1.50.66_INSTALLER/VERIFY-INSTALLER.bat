@echo off
REM =====================================================================
REM  DateERP 1.50.66 - VERIFY INSTALLER PACKAGE
...
if exist "%PUBLISH%\appsettings.json" (
  powershell -NoProfile -Command "try { $j=Get-Content '%PUBLISH%\appsettings.json' -Raw | ConvertFrom-Json; if ($j.AppSettings.Version -eq '1.50.66') { exit 0 } else { exit 1 } } catch { exit 1 }" >nul 2>&1
  if !errorlevel!==0 (
...
if exist "%ROOT%\..\DateERP_1.50.66_INSTALLER\Config" (
...
if exist "%ROOT%\RUN.bat" (
  findstr /I "1.50.66" "%ROOT%\RUN.bat" >nul 2>&1
  if !errorlevel!==0 (
...
findstr /I /S "DateERP_Publish C:\\DateERP" "%ROOT%\INSTALL-1.50.66.bat" >nul 2>&1
if !errorlevel!==0 (
  REM This is expected to have safety checks, but should not have hardcoded old install path as target
  echo [INFO] INSTALL-1.50.66.bat mentions old paths only for safety checks (OK)
) 
REM Check if INSTALL-1.50.66.bat has correct install root
findstr /C:"DateERP_1.50.66" "%ROOT%\INSTALL-1.50.66.bat" >nul 2>&1
if !errorlevel!==0 (
  echo [OK] INSTALL-1.50.66.bat targets DateERP_1.50.66 isolated folder
) else (
  echo [FAILED] INSTALL-1.50.66.bat does not target 1.50.66 folder
  set /a ERRORS+=1
)
