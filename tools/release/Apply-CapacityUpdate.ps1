...
try {
    ...
    if ($base.version -ne '1.50.4' -or $target.version -ne '1.50.6') { throw 'Wrong base or target version.' }
    ...
    $staging = $null
    Write-Host "PASS: New DateERP 1.50.6 folder: $dest" -ForegroundColor Green
    Write-Host 'Original folder was not changed. No application was started and no database was accessed.'
    Write-Host 'Run Check-Capacity-WPF.cmd and Check-Print-WPF.cmd before using a BACKED-UP TEST database.'
